...
{
	public static ArrayList<Etudiant> construireTabEtudiant()
	{
		/*----------------------*/
		/*  Variable            */
		/*----------------------*/
	...
	public static ArrayList<Salle> construireTabSalle()
	{
		Decomposeur dec;
		Scanner     scFic;
	...
	public static ArrayList<Equipe> construireTabEquipe( ArrayList<Etudiant> tabEtudiant )
	{
		/*----------------------*/
		/*  Variable            */
		/*----------------------*/
		int[]  nbCategorie = Outil.getTabCategorie(tabEtudiant);
		...
		int tailleGrp   = Outil.getCapaEquipe();
		...
		for(int indiceCategorie = 0; indiceCategorie < tabCategorie.length; indiceCategorie++)
		{
			// Attribution de la catégorie actuel à la variable "categorie"

			categorie = (char)('A' + indiceCategorie);
			...
			while(indiceMembre < nbCategorie[indiceCategorie])
			{
				...
				for(int i = 0; i < 4; i++) { 
					tabEtudiant.get(indiceMembre+i).setEquipe(cptEquipe);
					tabMembreEquipe.add(tabEtudiant.get(indiceMembre+i));	
				}

				// Entrer d'un 5ème étudiant si necéssaire
				if(nbEquipe <= surplus) { 
					tabEtudiant.get(indiceMembre+4).setEquipe(cptEquipe);
					tabMembreEquipe.add(tabEtudiant.get(indiceMembre+4)); 
				}
				...
				// Suppression d'une place dans la salle actuel
				tabSalle.get(indiceSalle).moinsPlace();       
				
				// Réinitialisation de la liste des membres de l'équipe
	...
	public static ArrayList<Jury> construireTabJury ( ArrayList<Etudiant> tabEtudiant)
	{
		/*----------------------*/
		/*  Variable            */
		/*----------------------*/
		...
		/*----------------------*/
		try{
			// Lecture du fichier jury.data dans le dossier data
			...
			// Récupèration du temps d'un passage, et des pauses
			...
			{	
				...
				// Ajout du "end", pour pouvoir facilement identifier la fin de la liste des professeurs
				ligne  += "\tend";
				dec    	= new Decomposeur(ligne);
				salle  	= dec.getInt(1);
				...
				// Tant qu'on ne lit pas "end", on rajoute le nom des professeurs dans la liste
				...
				// Récupèration combien de minute le jury possède
				creneau   = Outil.getMinute(fin) - Outil.getMinute(debut);
				creneau   = creneau - passage;
				nbPassage = 1;
				
				// Comptage le nombre de passage que le Jury peut effectuer
				while(creneau >= 0)
				{
